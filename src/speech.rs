...
use anyhow::Result;
use asr::embed::{
    spawn_session, SessionConfig, SessionControl, SessionEvent as ToonSessionEvent, SessionHandle,
...
impl SpeechSession {
    ...
    pub fn set_auto_vad_enabled(&self, enabled: bool) {
        let _ = self
            .handle
            .control(SessionControl::SetAutoVadEnabled(enabled));
    }
...
pub fn spawn_speech_session(
    ...
    let sink: Arc<dyn ToonSessionSink> = Arc::new(ForwardingSink {
        session_id,
...
impl ToonSessionSink for ForwardingSink {
    fn on_event(&self, event: ToonSessionEvent) {
        let mapped = match event {
            ToonSessionEvent::SessionStarted => SpeechEvent::SessionStarted {
                session_id: self.session_id,
            },
            ToonSessionEvent::Listening => SpeechEvent::Listening {
                session_id: self.session_id,
            },
            ...
            ToonSessionEvent::FinalText { turn_id, text } => SpeechEvent::FinalText {
                session_id: self.session_id,
                turn_id,
                text,
            },
            ToonSessionEvent::SessionEnded => SpeechEvent::SessionEnded {
                session_id: self.session_id,
            },
            ToonSessionEvent::Error { message } => SpeechEvent::Error {
                session_id: self.session_id,
                message,
            },
            ToonSessionEvent::Status { state, detail } => SpeechEvent::Status {
                session_id: self.session_id,
                state,
                detail,
            },
