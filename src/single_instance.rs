...
pub fn acquire_primary_instance_lock() -> Result<SingleInstanceGuard, SingleInstanceError> {
    ...
    if let Some(parent) = lock_path.parent() {
        fs::create_dir_all(parent).map_err(|source| SingleInstanceError::LockIo {
    ...
    let lock_file = OpenOptions::new()
        ...
        .write(true)
        .open(&lock_path)
        .map_err(|source| SingleInstanceError::LockIo {
    ...
    if rc != 0 {
        ...
        return Err(SingleInstanceError::LockIo {
            path: lock_path,
            source,
        });
    }
...
mod tests {
    ...
    fn default_lock_path_uses_application_support_dir() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/Users/spence/Library/Application Support/Azad/instance.lock")
        );
    }
