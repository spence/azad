    ...
    pub fn reduce(&mut self, input: InteractionInput) -> Vec<InteractionEffect> {
        match input {
            InteractionInput::HoldPressed { now_ms, snapshot } => {
                let is_double_tap = self.last_hold_press_at_ms.is_some_and(|last| {
                    now_ms.saturating_sub(last) <= self.config.double_tap_window_ms
                });
...
mod tests {
    ...
    fn double_tap_from_idle_toggles_mode_without_starting_hold() {
        ...
        assert_eq!(
            second_press,
            vec![InteractionEffect::InterruptAndToggleAlwaysListening]
        );
    }
    ...
    fn speech_idle_clears_manual_finalize_pending() {
        ...
        let _ = sm.reduce(InteractionInput::SpeechIdle {
            manual_hold_active: false,
        });
        assert!(!sm.manual_finalize_pending());
    }
    ...
    fn finalize_pressed_requires_overlay_visibility() {
        ...
        let hidden = sm.reduce(InteractionInput::FinalizePressed {
            overlay_visible: false,
        });
        assert!(hidden.is_empty());
        assert!(!sm.manual_finalize_pending());

        let visible = sm.reduce(InteractionInput::FinalizePressed {
    ...
    fn menu_toggle_does_not_mutate_interaction_state() {
        ...
        let _ = sm.reduce(InteractionInput::FinalizePressed {
            overlay_visible: true,
        });
        assert_core_state(&sm, Some(1000), true, true);
    ...
    fn speech_idle_clears_pending_only_when_hold_is_not_active() {
        ...
        let effects = sm.reduce(InteractionInput::SpeechIdle {
            manual_hold_active: true,
        });
        assert!(effects.is_empty());
        assert_core_state(&sm, Some(1000), true, true);
